...
void sendStatusDistIR()
{
  const int MRL = 64;
  char reply[MRL];
  snprintf(reply, MRL, "irc %.3f %.3f %.3f %.3f  %lu %lu %ld %ld %ld %ld %d %d\r\n" ,
           irDistance[0], irDistance[1],
           irDistance[2], irDistance[3],
           irRaw[0], irRaw[1],
           irCal20cm[0], irCal80cm[0],
           irCal20cm[1], irCal80cm[1],
           useDistSensor,
...
void estimateIrDistance()
{
  ...
  { // dist sensor has power, so estimate
    ...
    irDistance[2] = tof_front.readRangeSingleMillimeters() / 1000.0;
    irDistance[3] = tof_front.readRangeSingleMillimeters() / 1000.0;
    // debug
//     if (hbTimerCnt %200 == 0)
//     {
//       const int MSL = 70;
